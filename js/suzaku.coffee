...
class AjaxManager extends EventEmitter
  ...
  addGetRequest:(url,data,success,contentType)->
    return @addRequest
      type:"GET"
      url:url
      data:data
      ...
      retry:2
      success:success
  addPostRequest:(url,data,success,contentType)->
    return @addRequest
      type:"POST"
      url:url
      data:data
    ...
    for param,index in @params
      if param.value
        ...
        if param.force and noarg
          return console.warn "api #{@name} has different args from declare:",arg,param.name
        data[param.name] = arg
        argIndex += 1
    console.log "api #{@method} #{@name} send data:",data if debug
    if @method is "post" and @JAjaxOptions.json isnt false
      contentType = "application/json"
      data = JSON.stringify data
    opt =
      type:@method
      url:@url
      contentType:contentType or undefined
      data:data
      ...
      error:(req,textStatus,error)=>
        if typeof @onfail is "function"
          ...
          console.error "Api #{@method} error:#{error} --Suzaku.API"
          console.warn arguments
        @errorHandlers.all(req,textStatus,error) if @errorHandlers.all
        if @errorHandlers[req.status]
          @errorHandlers[req.status] req,textStatus,error
        evtData =
          successed:false
          JAjaxReq:req
          textStatus:textStatus
